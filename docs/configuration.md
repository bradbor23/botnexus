...
| `ToolEnvironmentPassThrough` | string[] | _(none)_ | Extra environment variable names exposed to `bash` and `exec` child processes. Those tools build the child environment from **empty** — a fixed allow-list (`PATH`, `HOME`, `SHELL`, `TERM`, `TMPDIR`, `LANG`, `TZ`, the XDG and profile roots, and the OS identity names), none of which authenticate anything. Name a variable here when a command genuinely needs it (`DOTNET_ROOT`, `NODE_OPTIONS`, a proxy setting). Per-name on purpose: there is no switch restoring wholesale inheritance, because that is the vulnerability rather than a setting. **Never name a variable that carries a credential** — it is handed to every agent that can run a command. See [Secrets and locations](user-guide/secrets-and-locations.md#what-an-agents-shell-commands-can-see). |
| `Memory.SharedStores` | object[] | _(none)_ | Named memory stores more than one agent can reach, each with its own `readers`/`writers` access list. Absent or empty means every agent's memory stays private, which is the default. Fields and the read/write asymmetry are documented in [Shared memory stores](development/workspace-and-memory.md#shared-memory-stores); `GET /api/memory/shared` reports the resulting access. |
